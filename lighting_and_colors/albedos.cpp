...
int main()
{
	...
	// ----------------------------------
	Shader myShader("../Data/shaders/shader_cl_test_albedos.vert", "../Data/shaders/shader_cl_test_albedos.frag");
	...
	unsigned int diffuseTxId{};
	glGenTextures(1, &diffuseTxId);
	glBindTexture(GL_TEXTURE_2D, diffuseTxId);
	...
	unsigned char* data = stbi_load("../data/textures/container2.png", &txWidth, &txHeight, &txChannels, 0);
	if (data)
	{
	...
	unsigned int specTxId{};
	glGenTextures(1, &specTxId);
	glBindTexture(GL_TEXTURE_2D, specTxId);
	...
	data = stbi_load("../data/textures/container2_specular.png", &txWidth, &txHeight, &txChannels, 0);
	if (data)
	{
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, txWidth, txHeight, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);	// push the texture into the texture object which is currently bound
		glGenerateMipmap(GL_TEXTURE_2D);	// automatically generate the mipmap for us
	...
	myShader.use();
	myShader.setInt("material.diffuse_albedo", 0);
	myShader.setInt("material.specular_albedo", 1);
